    ...
    <div class="container">
      ...
      <el-table ref="multipleTable" size="mini" border style="width: 100%" height="550px" :data="data" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="歌曲管理" width="110" align="center">
          <template slot-scope="scope">
            <el-button size="mini" @click="songEdit(scope.row.id,scope.row.singerId,scope.row.name)">歌曲管理</el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  // 监控
  watch: {
    // 搜索框中的内容发生变化时，搜索结果table列表的内容也跟着发生变化
  ...
  methods: {
    ...
    uploadUrl (id) {
      ...
      // return `/singer/avatar/update?id=${id}`
    },
    // 添加歌手
    addsinger () {
      ...
      params.append('pic', '/img/true.jpg')
    ...
    // 跳转到歌曲管理页
    songEdit (id,singerId,name) {
      this.$router.push({path: `/song`, query: {id: id, singerId: singerId,name: name}})
    }
  }
}
</script>
