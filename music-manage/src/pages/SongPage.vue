<template>
  <div class="table">
    ...
    <el-dialog title="添加歌曲" :visible.sync="centerDialogVisible" width="400px" center>
      <el-form  :model="registerForm" id="tf">
        <div>
          <label>歌曲名</label>
...
export default {
  ...
  methods: {
    ...
    // 更新歌曲图片
    uploadUrl (id,singerId) {
      return `${this.$store.state.HOST}/song/img/update?id=${id}&singerId=${this.singerId}`
    },
    // 更新歌曲url
    uploadSongUrl (id) {
      // return `${this.$store.state.HOST}/song/url/update?id=${id}&singerId=${this.singerId}`
      return `${this.$store.state.HOST}/song/url/update/${id}/${this.singerId}`
    },
    ...
    deleteRow () {
      // 删除歌曲
      deleteSong(this.singerId,this.idx)
        .then(response => {
          if (response) {
            this.getData()
