...
// export const loginIn = (params) => post(`user/login/check`, params)
// 注册
export const SignUp = (params) => post(`user/add`, params)
// 更新用户信息
export const updateUserMsg = (params) => post(`user/update`, params)
// 返回指定ID的用户
export const getUserOfId = (id) => post(`user/detail?id=${id}`)
...
// 返回歌单里指定歌单ID的歌曲
export const getListSongOfSongId = (songListId) => get(`listSong/detail/${songListId}`)
...
// 通过性别对歌手分类
export const getSingerOfSex = (sex) => get(`singer/q/sex?sex=${sex}`)
...
// 获取指定歌单的评分
export const getRankOfSongListId = (songListId) => get(`rank/q?songListId=${songListId}`)
